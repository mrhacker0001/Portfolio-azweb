...
function Portfolio() {
    const data = [
        { img: img1, nomi: "Sabio Price Attack", tavsif: "Happy Cyber Monday, trader! SabioTrade’s conditions were hacked by unknown intruders." },
        {
            img: img2, nomi: "Flower Shop", tavsif: "Flowers, 🌻 what the world needs Browse between hounders of flowers"},
            ...
            <div className="bottom-cards">
{
    data.map((item,index)=>(
        <div key={index} className="incard">
            ...
            <h1>{item.nomi}</h1>
            <p>{item.tavsif}</p>
            <button>View In Dribbble <img src={right} alt="" /></button>
        </div>
    ))
}
            </div>
